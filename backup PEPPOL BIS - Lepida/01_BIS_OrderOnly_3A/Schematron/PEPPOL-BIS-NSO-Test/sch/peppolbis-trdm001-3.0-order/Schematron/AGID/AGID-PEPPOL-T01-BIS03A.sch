...
<pattern id="INT-UBL-T01" xmlns="http://purl.oclc.org/dsdl/schematron">
	...
	<rule context="//cbc:EndpointID" flag="fatal">
		<assert test="(@schemeID='9921' and matches(.,'^[a-zA-Z0-9]{6,7}$')) or @schemeID!='9921'" flag="fatal" id="INT-T01-R026">[INT-T01-R026] - Se l'identificatore di endpoint si basa sullo schema di identificatori IT:IPA (ICD: 9921), questo dovrà seguire la sintassi [A-Z0-9]{6,7}.</assert>
	...
	<rule context="/ubl:Order"> 
		<assert test="cbc:CustomizationID = 'urn:fdc:peppol.eu:poacc:trns:order:3:extended:urn:www.ubl-italia.org:spec:ordine:ver3.1' and cbc:ProfileID = 'urn:fdc:peppol.eu:poacc:bis:order_only:3'" flag="fatal" id="INT-T01-R003">[INT-T01-R003] - La transazione dell'Ordine DEVE essere identificata dai seguenti identificativi: 

CustomizationID = 'urn:fdc:peppol.eu:poacc:trns:order:3:extended:urn:www.ubl-italia.org:spec:ordine:ver3.1', 

ProfileID = 'urn:fdc:peppol.eu:poacc:bis:order_only:3'.</assert>
	...
	<rule context="//cac:BuyerCustomerParty/cac:Party" flag="fatal"> 
		...
		<assert test="count(cac:PartyIdentification/cbc:ID)=1 and cac:PartyIdentification/cbc:ID/@schemeID='9921'" flag="fatal" id="INT-T01-R032">INT-T01-R032 - Un ordine deve riportare obbligatoriamente un solo identificatore della parte di business (party identification) del Buyer. Nel caso di Buyer di tipo PA deve riportare lo schema IPA.</assert>
		<assert test="cac:PartyIdentification/cbc:ID[@schemeID='9921'][string-length(translate(.,'ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789','')) = 0 and string-length() = 6]" flag="fatal" id="INT-T01-R033">INT-T01-R033 - Un ordine DEVE contenere Il codice UFE IPA dell'ufficio della PA che lo ha emesso, composto da 6 caratteri ASCII alfanumerici maiuscoli.</assert>
	</rule>
	
	<rule context="//cac:SellerSupplierParty/cac:Party" flag="fatal"> 
		<assert test="cac:PartyIdentification/cbc:ID[@schemeID='9921'][string-length(translate(.,'ABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789','')) = 0 and string-length() = 7]" flag="fatal" id="INT-T01-R036">INT-T01-R036 - Un ordine DEVE contenere Il codice destinatario (CODDEST) del fornitore, composto da 7 caratteri ASCII alfanumerici maiuscoli.</assert>
	</rule>
